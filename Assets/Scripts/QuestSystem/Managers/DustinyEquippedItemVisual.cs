...
/// 규칙:
/// - 전체 아이템 중 하나만 표시됩니다.
/// - Body_Sleepwear는 본과 머터리얼을 수정하지 않습니다.
/// - 슬립웨어는 기존 모습 그대로 Durry_Main을 따라갑니다.
...
public class DustinyEquippedItemVisual : MonoBehaviour
{
    ...
    [Tooltip(
        "슬립웨어는 본과 머터리얼을 건드리지 않고 " +
        "오브젝트 전체를 Durry_Main에 붙입니다."
    )]
    [SerializeField]
    ...
    /// <summary>
    /// 슬립웨어는 SkinnedMeshRenderer, bones, rootBone,
    /// mesh, materials를 전혀 수정하지 않습니다.
    ///
    /// 오브젝트 전체를 Durry_Main 밑으로 이동시켜
    /// 더리의 위치와 회전만 따라가게 합니다.
    /// </summary>
        ...
        Transform sleepwearRoot)
    {
        ...
        if (!keepSleepwearOriginalRenderer)
        {
            return;
        }
        ...
        if (sleepwearRoot == target ||
            target.IsChildOf(sleepwearRoot))
        {
            return;
        }

        if (sleepwearRoot.parent != target)
        {
        ...
        if (logAttachmentResults)
        {
            Debug.Log(
                $"[슬립웨어 부착] " +
                $"{sleepwearRoot.name} → {target.name} " +
                $"(머터리얼 및 본 변경 없음)",
                sleepwearRoot
            );
        }
