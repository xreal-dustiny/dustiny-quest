...
public class DustinyWaistNavFollow : MonoBehaviour
{
    ...
    [Tooltip("네비게이션을 욕조 앞에 그리되, 손/도구는 네비게이션보다 앞에 보이게 합니다.")]
    public bool drawTrackedHandsOverNav = true;
    ...
    private bool handsOverNavApplied;
    private const int NavUnderHandsRenderQueue = 3100;
    private const int NavAboveEverythingRenderQueue = 5500;
    private const int HandOverNavRenderQueue = 4500;
    ...
    private Coroutine forceShowCoroutine;
...
private void Start()
    {
        ...
        EnsureHandsDrawOverNav();
        PullNavInFrontOfOccluders();

        // DustinyDemoFlow.Start와 같은 프레임에 gaze-hide가 달리면
        // 재접속 직후 네비가 바로 꺼진 것처럼 보입니다. 한 프레임 뒤에 판정합니다.
        StartCoroutine(RefreshVisibilityNextFrame());
    }

    private System.Collections.IEnumerator RefreshVisibilityNextFrame()
    {
        yield return null;
...
private void LateUpdate()
    {
        ...
        // 핸드 머티리얼이 프레임마다 갱신되는 경우에도 네비 위에 손이 오도록 유지합니다.
        if (alwaysDrawInFront && drawTrackedHandsOverNav)
        {
            EnsureHandsDrawOverNav();
        }
    ...
    /// <summary>
    /// 재접속/씬 복귀 직후 네비를 반드시 한 번 보이게 한 뒤, 고개 숙임 규칙으로 되돌립니다.
    /// </summary>
    public void ForceShowThenResumeGaze(float forcedVisibleSeconds = 2.5f)
    {
        AutoFindReferences();
        ...
        forceShowCoroutine = StartCoroutine(ForceShowThenResumeGazeRoutine(forcedVisibleSeconds));
    }

    private System.Collections.IEnumerator ForceShowThenResumeGazeRoutine(float forcedVisibleSeconds)
    {
        float seconds = Mathf.Max(0.5f, forcedVisibleSeconds);
...
public void RefreshVisibilityImmediately()
    {
        ...
        // Look-down gating off => always show (do not leave bar stuck hidden).
        if (!enableLookDownToOpen)
        {
...
private void ApplyUiAlwaysInFront()
    {
        if (uiFrontMaterialApplied || waistNavRoot == null || !alwaysDrawInFront)
        {
            return;
        }

        // When hands-over is off (mini-game), still keep a sane transparent queue.
        // Extremely high sortingOrder previously made the bar effectively disappear on device.
        int navQueue = drawTrackedHandsOverNav
            ? NavUnderHandsRenderQueue
            : NavAboveEverythingRenderQueue;
        ...
        for (int i = 0; i < graphics.Length; i++)
        {
            ...
            ForceGraphicDrawInFront(graphic, navQueue);
        }
        ...
        if (canvas != null)
        {
            ...
            canvas.sortingOrder = drawTrackedHandsOverNav ? 5000 : 6000;
        }

        uiFrontMaterialApplied = true;
    }

public void ForceReapplyAlwaysInFrontMaterials()
    {
        uiFrontMaterialApplied = false;
        handsOverNavApplied = false;
        ApplyUiAlwaysInFront();
    }

/// <summary>
    /// Clears ZTest/render-queue material hacks and restores default UI draw state.
    /// Prefer positional pull-in-front over draw-order tricks for reliable visibility.
    /// </summary>
    public void ClearAlwaysInFrontDrawState()
    {
        alwaysDrawInFront = false;
        uiFrontMaterialApplied = false;
        handsOverNavApplied = false;
        ...
        for (int i = 0; i < graphics.Length; i++)
        {
            ...
            if (mat != null && mat.name.Contains("_NavAlwaysInFront"))
            {
            ...
            TMPro.TMP_Text tmp = graphic as TMPro.TMP_Text;
            if (tmp != null &&
                tmp.fontMaterial != null &&
                tmp.fontMaterial.name.Contains("_NavAlwaysInFront") &&
                tmp.font != null)
            {
...
private static void ForceGraphicDrawInFront(Graphic graphic, int renderQueue)
    {
        if (graphic == null)
        {
            return;
        ...
        Material source = graphic.material != null
            ? graphic.material
        ...
        Material front = new Material(source);
        front.name = source.name + "_NavAlwaysInFront";
        if (front.HasProperty("_ZTest"))
        {
            front.SetFloat("_ZTest", (float)CompareFunction.Always);
        }

        if (front.HasProperty("_ZWrite"))
        {
            front.SetFloat("_ZWrite", 0f);
        }

        front.renderQueue = Mathf.Max(3000, renderQueue);
        graphic.material = front;

        TMPro.TMP_Text tmp = graphic as TMPro.TMP_Text;
        if (tmp != null)
        {
            Material fontSource = tmp.fontSharedMaterial != null
                ? tmp.fontSharedMaterial
                : tmp.fontMaterial;
            if (fontSource != null)
            {
                Material fontFront = new Material(fontSource);
                fontFront.name = fontSource.name + "_NavAlwaysInFront";
                if (fontFront.HasProperty("_ZTestMode"))
                {
                    fontFront.SetFloat("_ZTestMode", (float)CompareFunction.Always);
                }
                else if (fontFront.HasProperty("_ZTest"))
                {
                    fontFront.SetFloat("_ZTest", (float)CompareFunction.Always);
                }

                if (fontFront.HasProperty("_ZWrite"))
                {
                    fontFront.SetFloat("_ZWrite", 0f);
                }

                fontFront.renderQueue = Mathf.Max(3000, renderQueue);
                tmp.fontMaterial = fontFront;
            }
        }
    }


private void EnsureHandsDrawOverNav()
    {
        if (!alwaysDrawInFront || !drawTrackedHandsOverNav)
        {
            return;
        ...
        BoostRenderersUnderName("LeftHandAnchor", HandOverNavRenderQueue);
        BoostRenderersUnderName("RightHandAnchor", HandOverNavRenderQueue);
        BoostRenderersUnderName("LeftHand", HandOverNavRenderQueue);
        BoostRenderersUnderName("RightHand", HandOverNavRenderQueue);
        BoostRenderersUnderName("Sponge", HandOverNavRenderQueue);
        BoostRenderersUnderName("Showering", HandOverNavRenderQueue);
        handsOverNavApplied = true;
    }


private static bool BoostRenderersUnderName(string rootName, int renderQueue)
    {
